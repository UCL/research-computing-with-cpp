...
1. Executable Size. 
    - The size of the compiled code can be increased by optimisation. Although one might expect code which is optimised to also be simplified and smaller, there are many optimisations which increase the size of the resulting machine code. An example of this would be **loop unrolling**. (See below)
2. Debugging Experience.
    - One of the most useful tools for debugging code is the ability to step through a program and check the values in variables as you execute line-by-line. Optimised code can make many changes, removing redundant variables, changing branching logic, restructuring or removing loops etc., that affect the correspondence between the C++ code and the compiled machine code. As a result, it may not always be possible to step through an optimised code, or meaningful to ask about the value of a specific variable at a particular point in the execution of a code. 
3. Compilation Time.
...
should be able to be calculated using SIMD registers when optimised, but 
...
**Don't worry about using modern C++ features like smart pointers and containers, but do turn the optimisations for production code!**

## Floating Point Arithmetic 

Floating point arithmetic is how we generally deal with approximating the real numbers in code. Floating point numbers can in principle have any level of precision, but the most common are:
...
- Identities from real arithmetic do not hold e.g. $(a + b) + c \neq a + (b + c)$ in floating point! 
...
Higher precision floating point numbers (like `double` as opposed to `float`) will give more accurate results when doing numerical work, but are also slower to perform calculations. `double` arithmetic takes more time, and if you are exploiting SIMD registers, fewer `double` values can fit in an available register. Some fast algorithms use single precision `float` or even half precision floating point numbers in areas where the results will not be significantly impact by this. You should always have tests for robustness and precision to check that these compromises are acceptable. 
...
If I have a large number $N$ and two much smaller numbers $a$ and $b$ and I want to calculate $N + a + b$. We know from the way that floating point numbers work that adding small and large numbers leads to rounding errors, so the best order to add these number is $(a+b) + N$, to keep the precision of $(a+b)$ and maximise the the size of the smaller number that we add to $N$. This is why re-associations in optimisation can cause a problem. Numerical algorithms with factors which compensate for rounding errors can have their error corrections optimised away by fast-math, because for _real_ numbers the error corrections would be zero and the compiler can identify them as redundant! 
...
- Use appropriate compiler optimisation flags when compiling for actual deployment. 

### Optional: Dumping Executables and Inspecting Assembly Code

This course is certainly not about assembly code, but you can understand something about what optimising compilers are doing if you take a look at the end result of your compiled code. This can be instructive when working with small examples to better understand what the compiler is doing to your code. 
...
- Registers on the CPU store data which you are working with. These include:
    ...
    - Registers for SIMD. 
- We won't have variable or types, just data in memory and in registers. This is much more closely aligned to how the machine actually works!
...
- If you assign a constant to a variable like `int x = 5`, you can usually spot this by finding the constant `$0x5` in a `mov` statement (`mov` moves a constant or a value contained in a memory location/register to a memory location or register). 
...
- The value in `%rax` which represents the loop iteration variable is now advanced by `0x10` (16), resulting in 25 iterations. This is what we might expect given that we are only using one SIMD register which can handle 4 32-bit integers at a time. 
