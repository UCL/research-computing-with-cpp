...
Smooth::Smooth(int sizex, int sizey, distance inner, int rank, int mpi_size, filling birth_1,
               filling birth_2, filling death_1, filling death_2, filling smoothing_disk,
               filling smoothing_ring)
    : sizey(sizey), inner(inner), rank(rank), mpi_size(mpi_size), birth_1(birth_1),
      birth_2(birth_2), death_1(death_1), death_2(death_2), smoothing_disk(smoothing_disk),
      smoothing_ring(smoothing_ring), outer(inner * 3), smoothing(1.0), frame(0),
      /// "Halo_Definitions"
      ...
      field2(new density[local_x_size_with_halo * sizey]), field(&field1), fieldNew(&field2) {
...
int Smooth::Range() { return range; }

int Smooth::Rank() { return rank; }

int Smooth::LocalXSize() { return local_x_size; }

int Smooth::LocalXSizeWithHalo() { return local_x_size_with_halo; }

int Smooth::Sizex() { return total_x_size; }
int Smooth::Sizey() { return sizey; }
int Smooth::Size() { return total_x_size * sizey; }
...
void Smooth::InitiateRightComms() {
  MPI_Isend(*field + sizey * local_x_min_needed_right, 1, halo_type, right, mpi_size + rank,
...
void Smooth::ResolveRightComms() { MPI_Wait(&request_right, MPI_STATUS_IGNORE); }
